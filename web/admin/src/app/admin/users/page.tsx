...
import AssignVenueButton from './AssignVenueButton'
  ...
  if (session.role !== 'nitecore') {
    return (
      <div className="p-8">
        ...
        <p className="text-sm text-gray-400">nitecore role required.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-8">
      ...
      <form method="GET" className="mb-6 flex gap-3">
        ...
        <button type="submit" className="rounded-lg bg-gray-700 hover:bg-gray-600 px-4 py-2 text-sm text-white">Search</button>
      </form>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-900 text-gray-400">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium">Venue ID</th>
          ...
          <tbody className="divide-y divide-gray-800">
            ...
            {users.map(u => (
              <tr key={u.user_id} className="bg-gray-950 hover:bg-gray-900">
                ...
                <td className="px-4 py-3 text-gray-500 font-mono text-xs">{u.venue_id || '—'}</td>
                <td className="px-4 py-3 text-right text-gray-400">{u.global_xp}</td>
                <td className="px-4 py-3 text-gray-400 text-xs">{new Date(u.created_at).toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  <AssignVenueButton userId={u.user_id} currentRole={u.role} />
                </td>
              </tr>
            ))}
          </tbody>
