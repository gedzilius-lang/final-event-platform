    ...
    <div className="p-8">
      ...
      <form method="GET" className="flex items-end gap-4 mb-8">
        {(['from', 'to'] as const).map(name => (
          <div key={name}>
            <label className="block text-xs text-gray-400 mb-1 capitalize">{name}</label>
            <input type="date" name={name} defaultValue={name === 'from' ? from : to}
        ...
        <button type="submit" className="rounded-lg bg-gray-700 hover:bg-gray-600 px-4 py-2 text-sm text-white">Apply</button>
      </form>
      ...
      {revenue && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <h2 className="text-sm font-medium text-gray-300 mb-1">Settlement estimate</h2>
            <p className="text-xs text-gray-500 mb-4">95% venue · 5% NiteOS platform fee</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-gray-500">Venue receives (CHF)</p>
                <p className="text-xl font-semibold text-green-400">{ncToCHF(Math.floor(revenue.total_orders_nc * 0.95))}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Platform fee (CHF)</p>
                <p className="text-xl font-semibold text-gray-300">{ncToCHF(Math.floor(revenue.total_orders_nc * 0.05))}</p>
              </div>
            </div>
          </div>
        </div>
