...
function NewItemForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      happy_hour_price_nc: form.get('happy_hour_price_chf')
        ? Math.round(Number(form.get('happy_hour_price_chf')) * 100)
        : undefined,
    ...
    try {
      ...
      router.push(`/catalog/items?venue_id=${venueId}`)
      router.refresh()
    } finally {
      setSaving(false)
    }
    ...
    <div className="p-8 max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input name="price_chf" type="number" step="0.01" min="0.01" required
            placeholder="8.00"
            className="w-full rounded-lg bg-gray-800 border border-gray-700 px-4 py-2.5
                       text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500"
          />
          <p className="mt-1 text-xs text-gray-500">1 CHF = 100 NC</p>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">Happy Hour Price (CHF, optional)</label>
          <input name="happy_hour_price_chf" type="number" step="0.01" min="0.01"
            placeholder="6.00"
            className="w-full rounded-lg bg-gray-800 border border-gray-700 px-4 py-2.5
                       text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500"
          />
