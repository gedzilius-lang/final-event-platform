  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {venueId && (
          <Link
            href={`/catalog/items/new?venue_id=${venueId}`}
      ...
      {!venueId && (
        <p className="text-sm text-gray-400 mb-4">
          ...
          <Link href="/catalog/venues" className="text-brand-100 underline">
            Venues
          </Link>
        </p>
      )}
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-900 text-gray-400">
            <tr>
              ...
              <th className="text-right px-4 py-3 font-medium">Price (NC)</th>
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                  {venueId ? 'No catalog items yet.' : ''}
                </td>
              </tr>
            )}
            {items.map(item => (
              <tr key={item.item_id} className="bg-gray-950 hover:bg-gray-900 transition-colors">
                <td className="px-4 py-3 text-xl">{item.icon ?? '—'}</td>
                <td className="px-4 py-3 text-white font-medium">{item.name}</td>
                <td className="px-4 py-3 text-gray-400">{item.category}</td>
                <td className="px-4 py-3 text-right text-gray-200">
                  {(item.price_nc / 100).toFixed(2)}
                </td>
                <td className="px-4 py-3 text-right text-gray-400">
                  {item.happy_hour_price_nc != null
                    ? (item.happy_hour_price_nc / 100).toFixed(2)
                    : '—'}
                </td>
                ...
                <td className="px-4 py-3">
                  <Link
                    href={`/catalog/items/${item.item_id}?venue_id=${venueId}`}
